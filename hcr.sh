...
function start_master {
    ...
    then
        ...
        docker rm $CONTAINER_NAME &>/dev/null
        echo "Now starting $CONTAINER_NAME ....."
        nohup $"docker" run --detach --name "hcr" --network host --hostname "hcrmaster" --privileged -e MODE=master -e MASTERIP=localhost -v "/dev/":"/dev/" -v "$CWD/":"/hcr/" -v "/bin/ping":"/bin/ping" -v "/usr/bin/telnet":"/usr/bin/telnet" --entrypoint "/hcr/entrypoint.sh" hcr:kinetic-ros &
        return 0
    else
        echo "Container $CONTAINER_NAME is still running. Please try to stop it first or do a restart instead."
        return 1
